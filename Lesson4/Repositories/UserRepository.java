...
import Lesson4.Interfaces.IUserRepo;
...
public class UserRepository implements IUserRepo {
    ...
    private UserRepository() {
        ...
        clients.add(new User(2, "Petr", "2222".hashCode(), 3));
        clients.add(new User(3, "Stan", "3333".hashCode(), 4));
        clients.add(new User(4, "Anna", "4444".hashCode(), 5));
    }
    ...
    public int create(String userName, int passwordHash, long cardNumber) throws RuntimeException {
        ...
        for (var currentClient : clients) {
            if (currentClient.getId() == client.getId()) {
                throw new RuntimeException("Такой клиент уже есть.");
            }
        }
        clients.add(client);
        return client.getId();
